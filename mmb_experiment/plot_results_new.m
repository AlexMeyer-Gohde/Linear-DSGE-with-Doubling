...
load('First_Run_HMS.mat')
HMS_Results_old=HMS_Results;
load('First_Run_HMS_add.mat')
Final_results=NaN(8,7,99);
Final_results(1:2,:,:)=HMS_Results_old(1:2,:,:);
Final_results(3:4,:,:)=HMS_Results(7:8,:,:);
Final_results(5:8,:,:)=HMS_Results_old(3:6,:,:);
HMS_Results=Final_results;

%model_variable_number=sum(squeeze(HMS_Results(1,1:4,:)));
model_variable_number=sum(squeeze(HMS_Results(1,3:4,:)));
...
diffs=abs(diffs);
diffs_select=0<diffs&diffs<1e-4;%1e-5;
diffs_select(1,:)=1;
...
legend('Cyclic Red','Log Red','Newton','Bernoulli','SF1','SF2','AutoUpdate','off','location','southeast')
...
legends={'Cyclic Red','Log Red','Newton','Bernoulli','SF1','SF2',};
...
legend('QZ','Cyclic Red','Log Red','Newton','Bernoulli','SF1','SF2','AutoUpdate','off','location','southeast')
ylabel('Density')
