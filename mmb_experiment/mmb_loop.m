...
doubling_options.P0=zeros( M_.ndynamic,M_.nspred);
...
HMS_Results(7,1,loop_k) = mean(total_time(ceil(length(total_time)*.2):ceil(length(total_time)*.8)));
if max(max(isnan(matrix_quadratic.X)))==0; HMS_Results(7,2,loop_k)=max(max(abs(X_dynare-matrix_quadratic.X)));end
HMS_Results(7,end,loop_k)=X_additional;
X_0_6=X(:,1:M_.nspred);
try
[errors] = dsge_practical_forward_errors_matrix_quadratic(matrix_quadratic);
HMS_Results(7,3:5,loop_k)=errors([4,7,8],1)';
catch
HMS_Results(7,3:5,loop_k)=NaN(1,3);
end
% if X_additional==newton_options.maximum_iter12,ations; 
% HMS_Results(6,:,loop_k)=NaN(1,7); 
% end
catch
HMS_Results(7,:,loop_k)=NaN(1,7);
end
...
HMS_Results(8,1,loop_k) = mean(total_time(ceil(length(total_time)*.2):ceil(length(total_time)*.8)));
if max(max(isnan(matrix_quadratic.X)))==0; HMS_Results(8,2,loop_k)=max(max(abs(X_dynare-matrix_quadratic.X)));end
HMS_Results(8,end,loop_k)=X_additional;
X_0_6=X(:,1:M_.nspred);
try
[errors] = dsge_practical_forward_errors_matrix_quadratic(matrix_quadratic);
HMS_Results(8,3:5,loop_k)=errors([4,7,8],1)';
catch
HMS_Results(8,3:5,loop_k)=NaN(1,3);
end
% if X_additional==newton_options.maximum_iter12,ations; 
% HMS_Results(6,:,loop_k)=NaN(1,7); 
% end
catch
HMS_Results(8,:,loop_k)=NaN(1,7);
end
